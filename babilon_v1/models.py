...
class MyUser(AbstractUser):
    active = models.BooleanField(
        verbose_name="Aktywny", null=True, blank=True, default=False
    )
    day_when_active = models.DateField(null=True, blank=True)
    profession = models.IntegerField(
        verbose_name="Stanowisko osoby", choices=JOB_STATUS, null=True, blank=True
    )
    ...
    phone_number = models.CharField(
        verbose_name="Numer telefonu", max_length=9, null=True, blank=True
    )
    ...
    rate_per_hour = models.FloatField(
        verbose_name="Stawka na godzinę", null=True, blank=True
    )
    rate_per_drive = models.FloatField(
        verbose_name="Stawka za wyjazd", null=True, blank=True
    )
    type_of_employment = models.IntegerField(
        verbose_name="Rodzaj zatrudnienia", choices=CONTRACT_TYPE, null=True, blank=True
    )
    driver_status = models.IntegerField(
        verbose_name="Status kierowcy",
    ...
    # @property
    def order_on_car(self):
        today_time = datetime.now().date()
        driver_active_order_count = (
            Orders.objects.filter(type_of_order="3")
            .filter(status="3")
            .filter(date=today_time)
            .filter(driver_id=self)
        ).count()
        return driver_active_order_count
    
    @property
    def orders_on_car_list(self):
        today_time = datetime.now().date()
        driver_active_order_list = (
            Orders.objects.filter(type_of_order="3")
            .filter(status="3")
            .filter(date=today_time)
            .filter(driver_id=self).order_by("date")
        )
        print(driver_active_order_list)
        return driver_active_order_list
...
class Address(models.Model):
    ...
    street = models.CharField(
        verbose_name="Ulica", null=True, blank=True, max_length=128
    )
    city = models.CharField(
        verbose_name="Miasto", null=True, blank=True, max_length=64, default="Kraków"
    )
    post_code = models.CharField(
        verbose_name="Kod pocztowy", null=True, blank=True, max_length=6
    )
    choice_count = models.IntegerField(verbose_name="Licznik wyboru", default=1)
...
class Clients(models.Model):
    ...
    client_name = models.CharField(
        verbose_name="Imię Nazwisko", max_length=64, null=True, blank=True
    )
    phone_number = models.CharField(verbose_name="Numer telefonu", max_length=15)

    status = models.IntegerField(
        verbose_name="Status klienta",
...
class WorkPlace(models.Model):
    workplace_name = models.CharField(verbose_name="Nazwa pizzerii", max_length=128)
...
class ProductSize(models.Model):
    size_name = models.CharField(verbose_name="Rozmiar produktu", max_length=64)

    pizza = models.BooleanField(
        verbose_name="Rozmiary dla pizzy?", null=True, blank=True
    )
    bottle = models.BooleanField(verbose_name="Rozmairy napoi?", null=True, blank=True)
    fast_food = models.BooleanField(
        verbose_name="Rozmiary fast_food?", null=True, blank=True
    )

    vege_topps_price = models.FloatField(
        verbose_name="Cena dodatku warzywnego", null=True, blank=True
    )
    beef_topps_price = models.FloatField(
        verbose_name="Cena dodatku mięsnego", null=True, blank=True
    )
    cheese_topps_price = models.FloatField(
        verbose_name="Cena dodatku serowego", null=True, blank=True
    )
    extra_topps_price = models.FloatField(
        verbose_name="Cena dodatku extra", null=True, blank=True
    )
...
class Category(models.Model):
    ...
    category_name = models.CharField(verbose_name="Kategoria menu", max_length=128)
...
class Products(models.Model):
    ...
    product_name = models.CharField(verbose_name="Nazwa produktu", max_length=128)

    pizza_number = models.IntegerField(
        verbose_name="Numer pizzy", null=True, blank=True, default=0
    )
    ...
    quantity = models.IntegerField(
        verbose_name="Ilosc", null=True, blank=True, default=1
    )
    toppings = models.ManyToManyField("Products", verbose_name="Składniki", blank=True)
    type_of_ingredient = models.IntegerField(
        verbose_name="Rodzaj składnika", choices=INGREDIENT_TYPE, null=True, blank=True
    )
    pizza_premium = models.BooleanField(verbose_name="Pizza premium?", default=False)
    is_pizza_half = models.BooleanField(verbose_name="Pizza pol_na_pol?", default=False)
    is_pizza_freestyle = models.BooleanField(
        verbose_name="Pizza freestyle?", default=False
    )
    extra_price = models.FloatField(
        verbose_name="Cena dodatków", null=True, blank=True, default=0
    )
    price = models.FloatField(
        verbose_name="Cena podstawowa", null=True, blank=True, default=0
    )
    discount = models.IntegerField(
        verbose_name="Rabat", default=0, null=True, blank=True
    )
    ...
    pizza_topp = models.BooleanField(verbose_name="Dodatek do pizzy", default=False)
    dish_topp = models.BooleanField(verbose_name="Dodatek do dań", default=False)
    code_number = models.CharField(
        verbose_name="Kod produktu", max_length=128, default="", null=True, blank=True
    )
    ...
    def __str__(self):
        if self.pizza_topp != True and self.dish_topp != True:
            return (
                str(self.product_name)
                + ", "
                + str(self.product_size)
                + ", "
                + str(self.category)
                + ", "
                + str(self.price)
...
class Vat(models.Model):
    vatrate = models.IntegerField(verbose_name="Stawka VAT", null=True, blank=True)
...
class Orders(models.Model):
    ...
    number = models.CharField(
        verbose_name="Numer zamówienia", max_length=64, null=True, blank=True
    )
    status = models.IntegerField(
    ...
    workplace_id = models.ForeignKey(
        "Workplace",
        on_delete=models.CASCADE,
    ...
    type_of_order = models.IntegerField(
        verbose_name="Rodzaj zamówienia", choices=DELIVERY_TYPE, default=1
    )

    address = models.ForeignKey(
        "Address", on_delete=models.CASCADE, verbose_name="Adres", null=True, blank=True
    )

    pay_method = models.IntegerField(
        verbose_name="Płatność zamówienia", choices=PAY_METHOD, default=1
    )

    start_delivery_time = models.TimeField(
        verbose_name="Czas wywozu", blank=True, null=True
    )
    time_delivery_in = models.TimeField(
        verbose_name="Czas dostarczenia", blank=True, null=True
    )
    ...
    discount = models.IntegerField(
        verbose_name="Rabat", null=True, blank=True, default=0
    )
    info = models.CharField(
        verbose_name="Informacje", max_length=256, null=True, blank=True, default=""
    )
        ...
        if self.discount > 0:
            return round(
                (
                    ...
                    - (self.order_total_price * (self.discount / 100))
                ),
                2,
            )
    ...
    def __str__(self):
        return str(self.id) + " - " + str(self.number) + " - " + str(self.workplace_id)
...
class PositionOrder(models.Model):
    ...
    product_id_pizza_right = models.ForeignKey(
        "Products",
        on_delete=models.CASCADE,
    ...
    pizza_half = models.BooleanField(verbose_name="Pizza pol_na_pol", default=False)
    ...
    cake_info = models.CharField(
        verbose_name="Zmiany ciasta", max_length=128, null=True, blank=True, default=""
    )
    sauces_free = models.CharField(
        verbose_name="Sosy darmowe", max_length=256, null=True, blank=True, default=""
    )
    sauces_pay = models.CharField(
        verbose_name="Sosy płatne", max_length=256, null=True, blank=True, default=""
    )
    extra_price = models.FloatField(
        verbose_name="Cena dodatków", null=True, blank=True, default=0
    )
    extra_price_left = models.FloatField(
        verbose_name="Cena dodatków na prawej", null=True, blank=True, default=0
    )
    ...
    def total_price(self):
        if self.discount > 0:
            return round(
                float(
                    (
                        (self.price)
                        - (float(self.price)) * (float(self.discount) * 0.01)
                        + float(self.extra_price)
                    )
                    * int(self.quantity)
                ),
                2,
        ...
        if self.discount > 0 and self.extra_price == 0:
            ...
            return round(
                ((float(self.extra_price) + float(self.price)) * int(self.quantity)), 2
            )
    ...
    def __str__(self):
        if self.pizza_half != True:
            return (
                str(self.product_id.product_name)
                + ", "
                + str(self.product_id.product_size)
                + ", "
                + str(self.order_id)
            )
        else:
            return (
                str(self.halfpizza_name)
                + ", "
                + str(self.product_id.product_size)
                + ", "
                + str(self.order_id)
...
class Purchases(models.Model):
    ...
    purchase_name = models.CharField(
        verbose_name="Nazwa", max_length=256, null=True, blank=True, default=""
    )
    pay_method = models.IntegerField(
        verbose_name="Rodzaj wydatku: ", choices=PAY_METHOD_2, default="1"
    )

    price = models.FloatField(verbose_name="Cena",)
    type_purchases = models.IntegerField(
        verbose_name="Rodzaj kosztu: ", choices=TYPE_OF_PURCHASE, default="0"
    )

    info = models.CharField(
        verbose_name="Info", max_length=256, null=True, blank=True, default=""
    )
    ...
    def __str__(self):
        return str(self.id) + ", " + str(self.purchase_name) + ", " + str(self.price)
