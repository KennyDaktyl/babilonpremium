...
class LocalStatusView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if worker.profession == 0:
            ...
            total_orders=Orders.objects.filter(date__gte=date_start).filter(date__lte=date_end).filter(workplace_id=pizzeria_active).count
            income=Orders.objects.filter(workplace_id=pizzeria_active).filter(date__gte=date_start).filter(date__lte=date_end).filter(status=4)
            income_trips=Orders.objects.filter(workplace_id=pizzeria_active).filter(date__gte=date_start).filter(date__lte=date_end).filter(type_of_order=3).filter(status=4).count
            income_online_1=Orders.objects.filter(workplace_id=pizzeria_active).filter(date__gte=date_start).filter(date__lte=date_end).filter(pay_method=3).filter(status=4).count()
            income_online_2=Orders.objects.filter(workplace_id=pizzeria_active).filter(date__gte=date_start).filter(date__lte=date_end).filter(pay_method=4).filter(status=4).count()
            income_online_3=Orders.objects.filter(workplace_id=pizzeria_active).filter(date__gte=date_start).filter(date__lte=date_end).filter(pay_method=5).filter(status=4).count()
            purhases=Purchases.objects.filter(work_place=pizzeria_active).filter(date__gte=date_start).filter(date__lte=date_end)
            income_sum=0
            purhases_sum=0
            cash = 0
            cards=0
            ...
            if order_in>0:
                for el in orders_in_workplace:
                    ...
                    if el.pay_method==5:
                        ...
                        online_3_in=float(online_3_in,2)
            if len(drivers)>0:
               for driver in drivers:
                    driver.counter_cursers=counter_cursers(driver,date_start, date_end, pizzeria_active)
                    driver.counter_cursers_cash=counter_cursers_cash(driver,date_start, date_end, pizzeria_active)
                    driver.counter_cursers_card=counter_cursers_card(driver,date_start, date_end, pizzeria_active)
                    driver.counter_cursers_card_count=counter_cursers_card_counter(driver,date_start, date_end, pizzeria_active)
                    driver.counter_cursers_online_1=counter_cursers_online_1(driver,date_start, date_end, pizzeria_active)
                    driver.counter_cursers_online_2=counter_cursers_online_2(driver,date_start, date_end, pizzeria_active)
                    driver.counter_cursers_online_3=counter_cursers_online_3(driver,date_start, date_end, pizzeria_active)
                    driver.save()
                    no_courses=False
            else:
                no_courses=True

            date_start = json.dumps(
                date_start, sort_keys=True, indent=1, cls=DjangoJSONEncoder)
            request.session['data_start'] = date_start
        ...
        if worker.profession == 1:
            ...
            ctx = {'worker': worker, 'setdate': setdate, 'date_start': date_start, 'pizzerias': pizzerias, 'form': form, 'pizzeria_active': pizzeria_active
                   }
            return TemplateResponse(request, "local_status.html", ctx)
        if worker.profession == 2:
            work_place = worker.work_place.all()
            ctx = {'worker': worker, 'work_place': work_place,
                   }
        ...
        if form.is_valid():
            ...
            try:
                ...
                if pizzeria_set_id:
                    ...
                    ctx = {'worker': worker, 'pizzeria_active': pizzeria_active, 'setdate': setdate, 'date_start': date_start, 'date_end': date_end
                           }
                    return TemplateResponse(request, "local_status.html", ctx)
                else:
            ...
            if pizzeria:
                ...
                pizzeria_set_id = request.session['pizzeria']
                pizzeria_active = WorkPlace.objects.get(pk=pizzeria_set_id)
                ctx = {'form': form, 'worker': worker, 'pizzeria_active': pizzeria_active,
                       }
                return TemplateResponse(request, "local_status.html", ctx)
...
class AddToppView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        topps = Products.objects.filter(pizza_topp=True).order_by('type_of_ingredient')
        ctx = {'form': form, 'topps': topps,'type_of_ingredient_list':type_of_ingredient_list
               }
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            new_topp = Products.objects.create(
                product_name=topp_name, type_of_ingredient=type_of_ingredient, pizza_topp=True)
...
class SetToppsPriceBySize(PermissionRequiredMixin, UpdateView):
    permission_required = 'babilon_v1.add_products'
    model = ProductSize
    fields = ['vege_topps_price','beef_topps_price','cheese_topps_price','extra_topps_price','extra_1_topps_price','extra_2_topps_price','extra_3_topps_price','extra_4_topps_price' ]
...
class SetToppPriceBySize(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        ctx = {'pizza_sizes': pizza_sizes, 'vege_topps': vege_topps,
               'beef_topps': beef_topps, 'cheese_topps': cheese_topps,'extra_topps_price':extra_topps_price,'extra_1_topps_price':extra_1_topps_price,'extra_2_topps_price':extra_2_topps_price,'extra_3_topps_price':extra_3_topps_price,'extra_4_topps_price':extra_4_topps_price }
...
class AddSauceView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        form = AddSauceForm()
        lista=[6,7]
        sauces=Products.objects.filter(type_of_ingredient__in=lista).order_by('type_of_ingredient')
...
class AddPizzaView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        pizza_sizes = ProductSize.objects.filter(pizza=True)
        ctx = {'form': form, 'pizzas': pizzas, 'pizza_sizes': pizza_sizes,
                   }
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for size in pizza_sizes:
                ...
                new_pizza.code_number=str(new_pizza.category.category_number)+str(pizza_number)+str(int(new_pizza.product_size.id)+2)
                new_pizza.save()
                for topp in toppings:
                    new_pizza.toppings.add(topp)
                    new_pizza.save()
                new_pizza.code_number=str(new_pizza.category.category_number)+str(new_pizza.pizza_number)+str(int(new_pizza.product_size.id)+2)
                new_pizza.save()
                i+=1    
...
class AddToppForDishView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        ctx = {'form': form, 'dish_topps': dish_topps, }
    ...
    def post(self, request):
        ...
        if form.is_valid():
            topp_name = form.cleaned_data['topp_name']
            price = form.cleaned_data['price']
            new_topp_dish = Products.objects.create(
                product_name=topp_name, dish_topp=True, price=price)
            return redirect('/local_status/')
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            new_category = Category.objects.create(
                category_name=category_name, display=True)
            return redirect('/local_status/')
...
class ProductsListView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        ctx = {'products': products, 'categorys': categorys,
               'sizes': sizes, }
...
class DelProductView(PermissionRequiredMixin, DeleteView):
    permission_required = 'babilon_v1.del_products'
    model = Products
    fields = '__all__'
...
class DelSessionPizzeriaView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        work_place = worker.work_place.all()
        pizzerias = WorkPlace.objects.all()
        ctx = {'worker': worker, 'pizzerias': pizzerias, 'form': form,
               }
        return TemplateResponse(request, "set_local.html", ctx)
...
class SetSessionDateView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        worker = request.user
        ctx = {'worker': worker, 'formD': formD, 'formT': formT, 'formM': formM,
               }
...
class SetTodayDateView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if formT.is_valid():
            ...
            date_start = json.dumps(
                today_show, sort_keys=True, indent=1, cls=DjangoJSONEncoder)
            request.session['date_start'] = date_start
            date_end = json.dumps(
                today_show, sort_keys=True, indent=1, cls=DjangoJSONEncoder)
            request.session['date_end'] = date_end
...
class SetMonthDateView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if formM.is_valid():
            ...
            date_start = datetime(year=int(year_now),
                                  month=int(month), day=1).date()

            last_day = date_start.replace(
                day=calendar.monthrange(year_now, date_start.month)[1])
            date_end = last_day
...
class DriverMobileView(PermissionRequiredMixin, View):
    ...
    def get(self, request,pk):
        ...
        for el in request.user.work_place.all():
            work_place_id=el.id
        print(work_place_id)
        my_orders=Orders.objects.filter(type_of_order=3).filter(driver_id=driver).filter(date__day=today).filter(date__month=month).filter(date__year=year).filter(workplace_id=work_place_id)
        orders=Orders.objects.filter(type_of_order=3).filter(status__in=[1,2]).filter(date__day=today).filter(date__month=month).filter(date__year=year).filter(workplace_id=work_place_id)
        for el in orders:
            print(el.workplace_id)
        ctx={"orders":orders,'my_orders':my_orders,'driver':driver}
        return render(request, "driver_mobile_view.html", ctx)

@method_decorator(login_required, name='dispatch')
class DriverMobileOrderView(PermissionRequiredMixin, View):
    permission_required = 'babilon_v1.view_orders'
    ...
    def get(self, request,pk):
        ...
        ctx={"order":order,'driver':driver,'positions_on_order':positions_on_order}
...
class DriverCoursesView(PermissionRequiredMixin, View):
    ...
    def get(self, request,pk):
        ...
        orders=Orders.objects.filter(type_of_order="3").filter(date__gte=date_start).filter(
            date__lte=date_end).filter(workplace_id=pizzeria_active).filter(
                driver_id=driver)
        orders_cash_count=Orders.objects.filter(type_of_order="3").filter(date__gte=date_start).filter(
            date__lte=date_end).filter(workplace_id=pizzeria_active).filter(
                driver_id=driver).filter(pay_method=1).count()
        orders_card_count=Orders.objects.filter(type_of_order="3").filter(date__gte=date_start).filter(
            date__lte=date_end).filter(workplace_id=pizzeria_active).filter(
                driver_id=driver).filter(pay_method=2).count()
        on_line_list=[3,4,5]
        orders_online_count=Orders.objects.filter(type_of_order="3").filter(date__gte=date_start).filter(
            date__lte=date_end).filter(workplace_id=pizzeria_active).filter(
                driver_id=driver).filter(pay_method__in=on_line_list).count()
        ...
        ctx = {'orders': orders,'orders_cash_count':orders_cash_count,'orders_card_count':orders_card_count,'orders_online_count':orders_online_count}
    ...
    def post(self, request,pk):
        ...
        if pay_method=="3":
            ...
            orders=Orders.objects.filter(type_of_order="3").filter(date__gte=date_start).filter(
            date__lte=date_end).filter(workplace_id=pizzeria_active).filter(
                driver_id=driver).filter(pay_method__in=pay_method_list)
        else:
            pay_method_list=[3,4,5]
            orders=Orders.objects.filter(type_of_order="3").filter(date__gte=date_start).filter(
            date__lte=date_end).filter(workplace_id=pizzeria_active).filter(
        ...
        if form.is_valid():
            ...
            for el in workers:
                if str(el.username) == str(username):
                    ...
                    ctx = {'drivers': drivers, 'form': form,'error_info': error_info}
                    return render(request, "drivers.html", ctx)
...
class DelDriverView(PermissionRequiredMixin,DeleteView):
    permission_required = 'babilon_v1.del_myuser'
    model = MyUser
    fields = '__all__'
    template_name_suffix = ('_confirm_delete')
    success_url = ('/drivers/')
        ...
        if form.is_valid():
            ...
            for el in workers:
                if str(el.username) == str(username):
                    ...
                    form = MenagerForm()
                    error_info = "Username istnieje"
                    ctx = {'menagers': menagers, 'form': form,'error_info': error_info}
                    return render(request, "menagers.html", ctx)
        ...
        if form.is_valid():
            ...
            for el in workers:
                if str(el.username) == str(username):
                    ...
                    form = MenagerForm()
                    error_info = "Username istnieje"
                    ctx = {'barmans': barmans, 'form': form,'error_info': error_info}
                    return render(request, "barmans.html", ctx)
...
class PurchaseView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        purchaeses = Purchases.objects.filter(
            work_place=pizzeria_active).filter(date__range=[date_start, date_end])
        total=0
        ...
        df = pd.DataFrame(list(purchaeses.values('id','date', 'work_place', 'purchase_name','pay_method','type_purchases','price','info')))
        df.index+=1
        df.to_excel(os.path.join(settings.MEDIA_ROOT, 'path_to_file.xlsx'))

        ctx = {'total':total,'purchaeses': purchaeses, 'form': form,'formReward':formReward,'formTax':formTax,'formConst':formConst,'formOther':formOther}
    ...
    def post(self, request):
        ...
        if formReward:
            if formReward.is_valid():
                ...
                p.purchase_name=str(worker.username)+": "+str(worker.first_name)+" "+str(worker.last_name)
...
class PurchaseCategoryView(PermissionRequiredMixin, View):
    ...
    def get(self, request,pk):
        ...
        purchaeses = Purchases.objects.filter(
            work_place=pizzeria_active).filter(date__range=[date_start, date_end]).filter(type_purchases=pk)
        ...
        ctx = {'total':total,'pk':pk,'purchaeses': purchaeses, 'form': form,'formReward':formReward,'formTax':formTax,'formConst':formConst,'formOther':formOther}
        return render(request, "purchaeses.html", ctx)
...
class DelPurchaseView(PermissionRequiredMixin, DeleteView):
    permission_required = 'babilon_v1.del_purchase'
...
class ProductsView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        pizza_sizes = ProductSize.objects.filter(pizza=True)
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        ctx={'order':order,'positions_on_order':positions_on_order,'products':products,'pizza_sizes':pizza_sizes,'categorys':categorys,'pizzeria':pizzeria,}
...
class CategoryProductsView(PermissionRequiredMixin, View):
    ...
    def get(self, request,pk):
        ...
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        if category.category_number==1 or category.category_number==2:
            pizza_sizes = ProductSize.objects.filter(pizza=True)
            ...
            ctx={'order':order,'positions_on_order':positions_on_order,'products':products,'categorys':categorys,'category':category,'pizzeria':pizzeria,}
    ...
    def post(self, request,pk):
        ...
        pizza_size = ProductSize.objects.get(pk=size_id)
        category=Category.objects.get(pk=pk)
        products=Products.objects.filter(category=category)
        ...
        pizza_sizes = ProductSize.objects.all()
        categorys=Category.objects.all()
        if category.category_name=="Pizza":
            products = Products.objects.filter(category=category).filter(product_size=pizza_size)
        else:
            products = Products.objects.filter(category=category)
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        
        if category.category_name=="Pizza":
            ctx={'order':order,'positions_on_order':positions_on_order,'products':products,'pizza_sizes':pizza_sizes,'pizza_size':pizza_size,'categorys':categorys,'category':category,'pizzeria':pizzeria,}
            return TemplateResponse(request, "pizza_menu.html",ctx)
        else:
            ctx={'order':order,'positions_on_order':positions_on_order,'products':products,'pizza_sizes':pizza_sizes,'categorys':categorys,'category':category,'pizzeria':pizzeria,}
            return TemplateResponse(request, "product_menu.html",ctx)
...
class AddNewOrderLocalView(PermissionRequiredMixin,View):
    ...
    def get(self, request):
        ...
        order.time_zero=datetime.now()+timedelta(minutes=int(default_time_zoro_local))
        order.address=None
        order.save()
        ...
        categorys=Category.objects.all()
        category_pizza = Category.objects.filter(category_name="Pizza")
        pizzas = Products.objects.filter(category__in=category_pizza)
        pizza_sizes = ProductSize.objects.filter(pizza=True)
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
...
class AddNewOrderOutsideDriverView(PermissionRequiredMixin,View):
    ...
    def get(self, request):
        ...
        order.barman_id = request.user
        order.number = new_number(pizzeria.id)
        order.time_zero=datetime.now()+timedelta(minutes=int(default_time_zoro_outside))
        order.save()
        request.session['active_orders'] = order.id
        products=Products.objects.all()
        categorys=Category.objects.all()
        ...
        ctx={'clients':clients,'clients_addresses':clients_addresses,'order':order,'positions_on_order':positions_on_order,'products':products,'pizzas':pizzas,'pizza_sizes':pizza_sizes,'categorys':categorys,'pizzeria':pizzeria,}
...
class AddPizzaToOrderView(PermissionRequiredMixin,View):
    ...
    def get(self, request,order,prod):
        ...
        cakes = Products.objects.filter(type_of_ingredient="5").order_by('product_name')
        souse_pay = Products.objects.filter(type_of_ingredient="6")
        souse_free = Products.objects.filter(type_of_ingredient="7")
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        
        ctx={'order':order,'pizza':pizza,'other_size':other_size,'positions_on_order':positions_on_order,'categorys':categorys,'cakes':cakes,'souse_pay':souse_pay,'souse_free':souse_free}
...
class AddPizzaFreestyleToppsView(PermissionRequiredMixin,View):
    ...
    def get(self, request, order, prod):
        ...
        vegetopps = Products.objects.filter(type_of_ingredient="1").order_by('product_name')
        beeftopps = Products.objects.filter(type_of_ingredient="2").order_by('product_name')
        cheesetopps = Products.objects.filter(type_of_ingredient="3").order_by('product_name')
        extra_list=[4,7,8,9,10,11]
        extratopps = Products.objects.filter(type_of_ingredient__in=extra_list).order_by('product_name')
        cake = Products.objects.filter(type_of_ingredient="5").order_by('product_name')
    ...
    def post(self, request, order, prod):
        ...
        return redirect('add_pizza_freestyle_to_order', order=order.id, pos=new_position.id)
...
class AddPizzaFreestyleToOrderView(PermissionRequiredMixin,View):
    ...
    def get(self, request,order,pos):
        ...
        other_size=Products.objects.filter(product_name=pizza.product_id.product_name)
        cakes = Products.objects.filter(type_of_ingredient="5").order_by('product_name')
        souse_pay = Products.objects.filter(type_of_ingredient="6")
        souse_free = Products.objects.filter(type_of_ingredient="7")
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        
        ctx={'order':order,'pizza':pizza,'product':product,'other_size':other_size,'positions_on_order':positions_on_order,'categorys':categorys,'cakes':cakes,'souse_pay':souse_pay,'souse_free':souse_free}
        return TemplateResponse(request, "add_pizza_freestyle_to_order.html",ctx)
    ...
    def post(self, request,order,pos):
        ...
        if 'delete' in request.POST:
            ...
            position_order.save()
            return redirect('add_pizza_freestyle_to_order', order=order.id, pos=position_order.id)
        ...
        if extra_price != None:
            ...
            position_order.extra_price = new_position.extra_price
            position_order.save()
        if info != "":
...
class ChangeToppsView(PermissionRequiredMixin,View):
    ...
    def post(self, request, order, prod):
        ...
        return redirect('add_modyfi_pizza_to_order', order=order.id, pos=position_order.id)
...
class AddModyfiToOrderView(PermissionRequiredMixin,View):
    ...
    def post(self, request,order,pos):
        ...
        if 'delete' in request.POST:
            ...
            new_position.save()
            return redirect('add_modyfi_pizza_to_order', order=order.id, pos=new_position.id )
...
class UpdatePizzaToOrderView(PermissionRequiredMixin,View):
    ...
    def get(self, request,order,pos):
        ...
        position_order.save()
        pos=position_order
        ctx={'order':order,'pos':pos,'pizza':pizza,'other_size':other_size,'positions_on_order':positions_on_order,'categorys':categorys,'cakes':cakes,'souse_pay':souse_pay,'souse_free':souse_free}
    ...
    def post(self, request,order,pos):
        ...
        if 'delete' in request.POST:
            ...
            position_order.discount=0
            position_order.save()
            return redirect('update_pizza_to_order', order=order.id, pos=position_order.id )
...
class UpdateProductToOrderView(PermissionRequiredMixin,View):
    ...
    def get(self, request,order,pos):
        ...
        position_order.save()
        pos=position_order
        ctx={'order':order,'pos':pos,'product':product,'other_size':other_size,'positions_on_order':positions_on_order,'categorys':categorys,'cakes':cakes,'souse_pay':souse_pay,'souse_free':souse_free}
    ...
    def post(self, request,order,pos):
        ...
        if 'delete' in request.POST:
            ...
            position_order.discount=0
            position_order.save()
            return redirect('update_product_to_order', order=order.id, pos=position_order.id )
...
class Secend_Half_Pizza_SearchView(PermissionRequiredMixin,View):
    ...
    def get(self, request, order,pk,size):
        ...
        souse_free = Products.objects.filter(type_of_ingredient="7")
        cakes = Products.objects.filter(type_of_ingredient="5").order_by('product_name')
        pizzas=Products.objects.filter(category=1).filter(product_size=size).filter(is_pizza_freestyle=False)
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        
        
        ctx={'order':order,'size':size,'pizzas':pizzas,'categorys':categorys,'cakes':cakes,'souse_pay':souse_pay,'souse_free':souse_free}
    ...
    def post(self, request, order, pk, size):
        ...
        if search != "":
            ...
            souse_free = Products.objects.filter(type_of_ingredient="7")
            cakes = Products.objects.filter(type_of_ingredient="5").order_by('product_name')
            pizzas=Products.objects.filter(product_size=size).filter(is_pizza_freestyle=False)
            pizzas=pizzas.filter(
            Q(pizza_number=search))
            positions_on_order=PositionOrder.objects.filter(order_id=order.id)
            
            ctx={'order':order,'size':size,'pizzas':pizzas,'categorys':categorys,'cakes':cakes,'souse_pay':souse_pay,'souse_free':souse_free}
            return TemplateResponse(request, "pizza_filter_for_secendhalf.html",ctx)
        else:
            if secend_half != "":
                order=Orders.objects.get(pk=order)
                pizza_left = Products.objects.get(id=pk)
                ...
                pizza_half.halfpizza_name="Lewa - " +str(pizza_left.product_name)+", Prawa - "+str(pizza_rigth.product_name)
                ...
                return redirect('left_and_right_halfpizza', pizza_half=pizza_half.id,order=order.id)
            else:
                pizza_left = Products.objects.get(id=pk)
                size=ProductSize.objects.get(pk=size)
                order=Orders.objects.get(pk=order)
                return redirect('add_secend_halfpizza',order=order.id,pk=pizza_left.id,size=size.id)
...
class Left_And_Right_Half_PizzaView(PermissionRequiredMixin,View):
    ...
    def get(self, request,pizza_half,order):
        ...
        ctx={'pizza_half':pizza_half,'order':order,'cakes':cakes,'positions_on_order':positions_on_order,'categorys':categorys,'souse_pay':souse_pay,'souse_free':souse_free}
    ...
    def post(self, request,pizza_half,order):
        ...
        if 'delete' in request.POST:
            ...
            pizza_half.save()
            return redirect('left_and_right_halfpizza', order=order.id, pizza_half=pizza_half.id)
        ...
        if extra_price != None:
            ...
            pizza_half.save()
            print(pizza_half.extra_price)
        else:
            pizza_half.extra_price = pizza_half.extra_price
            pizza_half.save()
            print(pizza_half.extra_price)
        if info != "":
...
class ChangeLeftToppsView(PermissionRequiredMixin,View):
    ...
    def post(self, request, order, pos):
        ...
        return redirect('update_halfpizza_to_order',pos=position_order.id,order=order.id)
...
class ChangeRightToppsView(PermissionRequiredMixin,View):
    ...
    def get(self, request, order, pos):
        order=Orders.objects.get(pk=order)
        new_position=PositionOrder.objects.get(pk=pos)
        product = Products.objects.get(id=new_position.product_id_pizza_right.id)
        new_position.cake_info=""
        new_position.change_topps = ""
        new_position.save()
    ...
    def post(self, request, order, pos):
        ...
        position_order.change_topps_info_other_side=input_del_topps +" "+input_add_topps
        position_order.order_id=order
        if position_order.extra_price_right>position_order.extra_price_left:
            position_order.extra_price=position_order.extra_price_right
        else:
...
class UpdateHalfPizzaToOrderView(PermissionRequiredMixin,View):
    ...
    def get(self, request,pos,order):
        ...
        ctx={'pizza_half':pizza_half,'order':order,'cakes':cakes,'positions_on_order':positions_on_order,'categorys':categorys,'souse_pay':souse_pay,'souse_free':souse_free}
        return TemplateResponse(request, "update_secendhalf_to_order.html",ctx)
    ...
    def post(self, request,order,pos):
        ...
        if 'delete' in request.POST:
            ...
            position_order.discount=0
            position_order.save()
            return redirect('update_halfpizza_to_order', order=order.id, pos=position_order.id )
        ...
        lista_id_rigth_topps=list(position_order.change_topps_info_other_side)
...
class ChangeLeftSidePizzaView(PermissionRequiredMixin,View):
    ...
    def get(self, request, order,pos):
        ...
        ctx={'order':order,'size':size,'pizzas':pizzas,'categorys':categorys,'positions_on_order':positions_on_order}
        return TemplateResponse(request, "pizza_filter_for_halfpizza_side_update.html",
                                ctx)
    def post(self, request, order, pos):
        search = request.POST.get('search')
        ...
        if search != "":
            ...
            ctx={'order':order,'size':size,'pizzas':pizzas,'categorys':categorys,'positions_on_order':positions_on_order}
            return TemplateResponse(request, "pizza_filter_for_halfpizza_side_update.html",ctx)
        else:
            if side_half!="":
                order=Orders.objects.get(pk=order)
                pizza_half = PositionOrder.objects.get(id=pos)
                ...
                pizza_half.halfpizza_name="Lewa - " +str(pizza_left.product_name)+", Prawa - "+str(pizza_half.product_id_pizza_right.product_name)
                pizza_half.price=price
                pizza_half.product_id=pizza_left
                # pizza_half.product_id_pizza_right=pizza_half.product_id_pizza_right.price
                pizza_half.save()
                
                return redirect('update_halfpizza_to_order', order=order.id, pos=pizza_half.id)
            else:
                pizza_half = PositionOrder.objects.get(id=pos)
                order=Orders.objects.get(pk=order)
                return redirect('change_left_side_pizza_to_order', order=order.id, pos=pizza_half.id)
        ...
        if search != "":
            ...
            if side_half!="":
                ...
                if price_right > price_left_price:
                    ...
                    price=price_left_price
                pizza_half.halfpizza_name="Lewa - " +str(pizza_half.product_id.product_name)+", Prawa - "+str(pizza_right.product_name)
                pizza_half.price=price
                ...
                pizza_half = PositionOrder.objects.get(id=pos)
                order=Orders.objects.get(pk=order)
                return redirect('change_right_side_pizza_to_order', order=order.id, pos=pizza_half.id)
...
class AddProductToOrderView(PermissionRequiredMixin,View):
    ...
    def get(self, request,order,prod):
        ...
        ctx={'order':order,'positions_on_order':positions_on_order,'product':product,'categorys':categorys,'souse_pay':souse_pay,'souse_free':souse_free}
...
class ProductsView(PermissionRequiredMixin,View):
    ...
    def get(self, request):
        ...
        category_pizza = Category.objects.filter(category_name="Pizza")
        positions_on_order=PositionOrder.objects.filter(order_id=order.id)
        ctx={'order':order,'positions_on_order':positions_on_order,'products':products,'categorys':categorys,'pizzeria':pizzeria,}
        return TemplateResponse(request, "main_menu.html",ctx)
    ...
    def get(self, request,pk):
        ...
        if request.is_ajax():
            ...
            if time_zero_ajax:
                ...
                order_finish.save()
                order_finish.time_zero=datetime.now() + timedelta(minutes=int(time_zero))
                # order_finish.save()
            if pay_method:
            ...
            if delivery_method:
                ...
                if delivery_method !="3":
                    order_finish.time_zero=datetime.now()+timedelta(minutes=int(default_time_zoro_local))
                    ...
                    order_finish.time_zero=datetime.now()+timedelta(minutes=int(default_time_zoro_outside))
...
class OrderChangeDeatailsView(PermissionRequiredMixin, View):
    ...
    def get(self, request,pk):
        ...
        if time_zero_manual:
            ...
            order_finish.time_zero=datetime.now()+timedelta(minutes=int(time_zero_manual))
            order_finish.barman_id=request.user
            # order_finish.save()
        order_finish.save()
        google_key=os.environ.get("GOOGLE_MAPS")
...
class OrdersView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        orders = Orders.objects.filter(date__day=today).filter(date__month=month).filter(date__year=year).filter(workplace_id=pizzeria_id)
        # for el in orders:
        #     el.active=False
        #     el.save()
        ...
        if request.is_ajax():
            try:
                ...
                if order_cancell_id:
                    ...
                    order_last=Orders.objects.filter(workplace_id=work_place_id).first()
                ...
                if order_close:
                    ...
                    order.closed=True
                    now=datetime.now()
                    order.time_delivery_in=datetime(year=now.year, month=now.month, day=now.day,hour=now.hour ,minute=now.minute)
                    order.save()
                    work_place_id=request.session['pizzeria']
                    order_last=Orders.objects.filter(workplace_id=work_place_id).first()
                    if order_last.id==order.id:
...
class OrdersFilterView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        pizzeria_id=request.session['pizzeria']
        lista=[1,2,3]
        orders_lokal_type = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(type_of_order="1").filter(status__in=lista).count()
        orders_outside_type = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(type_of_order="2").filter(status__in=lista).count()
        orders_driver_type = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(type_of_order="3").filter(status__in=lista).count()

        orders = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(active=True)
        orders_active = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(active=True).count()
        orders_prepare = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(status="2").count()
        orders_driver = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(status="3").count()
        orders_cancelled = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(status="5").count()
        orders_closed = Orders.objects.filter(date__day=today).filter(workplace_id=pizzeria_id).filter(status="4").count()
        
        in_progress=True
        drivers=MyUser.objects.filter(profession=4).filter(work_place=pizzeria_id).filter(active=True)
        ctx = {'orders': orders,'orders_active':orders_active,'orders_closed':orders_closed,'orders_prepare':orders_prepare,'orders_driver':orders_driver,'orders_cancelled':orders_cancelled,'orders_lokal_type':orders_lokal_type,'orders_outside_type':orders_outside_type,'orders_driver_type':orders_driver_type,'drivers':drivers,'in_progress':in_progress}
...
class UpdatePositionOrderView(PermissionRequiredMixin,UpdateView):
    ...
    model = PositionOrder
    fields = [
        'order_id','quantity','cake_info' ,'change_topps_info', 'change_topps_info_other_side','sauces_free', 'sauces_pay',
...
class ClientsView(PermissionRequiredMixin,View):
    ...
    def get(self, request):
        ...
        clients_addresses=Address.objects.all()
        form=ClientForm()
        ctx={'order':order,'clients':clients,'clients_addresses':clients_addresses,'form':form}
        return TemplateResponse(request, "clients.html",ctx)
    def post(self, request):
...
class DelAddressView(PermissionRequiredMixin,DeleteView):
    permission_required = 'babilon_v1.del_clients'
    model = Address
    fields = '__all__'
    template_name_suffix = ('_confirm_delete')
    success_url = ('/clients/')
